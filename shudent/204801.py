...
class map2048():``
    ...
    def get_score(self):
        ...
        return s
    #填充2到空位置,如果填充成功返回True,如果已满,则返回False
    def fill2(self):
                ...
                for r in self.data:
                    for c in range(self.__col -1):
                        if 0 == r[c]:
                            ...
                            r[c] = r[c + 1]
                            r [c + 1] = 0
            #判断是否发生碰撞,如果有碰撞则合并,合并结果靠左,右侧填充空格
            for r in self.data:
                for c in range(self.__col -1):
                    if r[c] == r[c + 1]:
                        moveflag = True
            ...
            for c in range(self.__col):
                for r in range(self.__row - 1):
                    if 0 == self.data[r][c]:
                        ...
                        self.data[r + 1][c] = 0
            #判断是否发生碰撞,如果有碰撞则合并,合并结果靠上,下面填充空格
            for c in range(self.__col):
                for r in range(self.__row - 1):
                    if self.data[r][c] == self.data[r + 1][c]:
                        moveflag = True
...
def on_key_down(event):
    ...
    updata_ui()
    if game.is_gameover():
        mb = messagebox.askokcancel(title="gameover",message="仆街\n是否推出游戏!")
        if mb:
            exit()
        else:
            game.reset()
